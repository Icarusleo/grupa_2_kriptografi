...
> **Kısa özet:** Projeye **MD5** ve **SHA-1** kriptografik hash fonksiyonları, **hiçbir kütüphane kullanılmadan** (kendi saf-Python implementasyonu — `hashlib` yok) eklendi. Eklenti (plugin) mimarisi sayesinde başka gruplar yeni hash fonksiyonlarını tek dosya bırakarak entegre edebilir.

| Algoritma | Özet | Blok | Tur | Çekirdek |
|---|---|---|---|---|
| MD5 | 128-bit | 512-bit | 64 | RFC 1321 |
| SHA-1 | 160-bit | 512-bit | 80 | FIPS 180-4 / RFC 3174 |
...
| `test_hashes.py` | **YENİ** — MD5 & SHA-1 için 10 bilinen-cevap (RFC) test vektörü + eklenti arayüzü doğrulaması; `python test_hashes.py` veya `pytest test_hashes.py` ile çalışır. |

### Frontend (`frontend/src/`)
| Dosya | Değişiklik |
|---|---|
| `types/algorithms.ts` | `AlgorithmDef`'e `isHash`, `digestBits`, `blockBits`, `rounds` alanları. |
| `api/hashApi.ts` | Backend metadata'sından `"Hash · 128-bit özet · 512-bit blok · 64 tur"` açıklaması üretiliyor; `Cryptographic Hash Functions` kategorisi; md5/sha1 için ikon+renk. Hash algoritmaları backend'den dinamik çekildiği için yeni hash eklemek frontend kodu gerektirmez. |
| `components/nodes/AlgorithmNode.tsx` | Hash algoritmaları için ayrı `HashNodeView`: ikon, `Digest / Block / tur / Key yok · IV yok` rozetleri, tek **Plaintext** girişi + **özet (digest)** çıkışı, `Girdi: N byte`, `<Algo> Özeti (HEX)` ve `Base64` alanları. |
...
Tarayıcıda `http://localhost:5173/` (port doluysa Vite 5174/5180… der). Sağdaki **"Algoritmalar"** panelinde **CRYPTOGRAPHIC HASH FUNCTIONS** başlığı altında **MD5 / SHA-1** görünür.
...
| `ABC…XYZabc…xyz0…9` (62 byte) | `d174ab98d277d9f5a5611c2c9f419d9f` | `761c457bf73b14d27e9e9265c46f4b4dda11f940` |
...
> Not: `dog` → `cog` tek harf farkı özetin tamamen değişmesini (çığ etkisi) gösterir; uzun girdiler (56 byte, 1 MB) çok-bloklu padding ve 64-bit uzunluk alanının doğruluğunu test eder.

### Otomatik test

`test_hashes.py` hem tüm bilinen-cevap vektörlerini hem de eklenti arayüzünü (`HashPlugin` sözleşmesi + `id/name/digest_size/block_size/rounds` metadata) doğrular.

```bash
cd backend/api
python test_hashes.py          # tabloyu basar
# veya
...
1. `backend/api/hash_plugins/` içine yeni bir dosya aç (örn. `sha256_plugin.py`).
2. `HashPlugin`'den türeyen bir sınıf yaz ve şunları doldur:
   - `id` → benzersiz kimlik (örn. `"sha256"`)
   - `name` → ekranda görünecek ad (örn. `"SHA-256"`)
...
3. (İsteğe bağlı) `test_hashes.py` içindeki `VECTORS` listesine kendi test vektörlerini ekle.
4. Backend'i yeniden başlat. Registry eklentiyi otomatik bulur (`[OK] Hash Plugin registered: ...`), frontend de `/api/v1/hash/algorithms`'tan çekip sidebar'a ekler. Ekstra kod gerekmez.
