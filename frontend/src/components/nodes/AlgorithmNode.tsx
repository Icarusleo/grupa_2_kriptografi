...
import type { CoreInputPreview } from '../../api/grain128api';
...
function RegisterDisplay({ bits, label, color }: { bits: number[]; label: string; color: string }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 2, flexWrap: 'nowrap', overflowX: 'auto', paddingBottom: 2 }}>
        {show.map((bit, i) => (
          <div
            key={i}
            style={{
              ...
              fontSize: 9, fontWeight: 700, fontFamily: 'monospace', borderRadius: 3,
...
function HexField({ label, value, color }: { label: string; value?: string; color: string }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{
        fontSize: 10, fontFamily: 'monospace', padding: '5px 8px', borderRadius: 5,
        background: '#181818', color, border: `1px solid ${color}44`,
...
function CoreInputSection({ cp }: { cp: CoreInputPreview }) {
  return (
    <div style={{
      background: '#161e16', border: '1px solid #2a4a2a', borderRadius: 6,
      padding: '8px 10px', marginBottom: 10,
      fontSize: 10, fontFamily: 'monospace', color: '#6b9a6b',
    }}>
      <div style={{ fontWeight: 700, color: '#86efac', marginBottom: 6, fontSize: 11 }}>
        NIST Core Input&nbsp;
        <span style={{ fontWeight: 400, color: '#4a7a4a' }}>m₀ = Encode(|ad|) ‖ ad ‖ plaintext ‖ 0x80</span>
      </div>
...
export function AlgorithmNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <BaseNode
      ...
      minWidth={440}
      helpText={algo.tagBits > 0
        ? `${algo.name} AEAD algoritması. Key (${algo.keyBits}-bit) ve IV (${algo.ivBits}-bit) bağlandıktan sonra plaintext'i şifreler ve ${algo.tagBits}-bit authentication tag üretir. Associated Data (AD) isteğe bağlıdır.`
        : `${algo.name} stream cipher. Key (${algo.keyBits}-bit) ve IV (${algo.ivBits}-bit) bağlandıktan sonra plaintext'i keystream XOR ile şifreler. Tag üretmez.`}
    >
      {/* Status banner */}
      {nodeData.processed && (
        <div style={{
          ...
          {nodeData.apiMessage ?? (nodeData.implemented ? 'Gerçek implementasyon' : 'Stub — backend bağlı değil')}
        </div>
      )}

      {/* Input / output port layout */}
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
        {/* Left: input badge statuses */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[
            { label: 'Key',                             ok: hasKey,       color: '#c4b5fd', optional: false },
            { label: `IV / Nonce (${algo.ivBits}-bit)`, ok: hasIV,        color: '#93c5fd', optional: false },
            ...
            >
              <span style={{
                width: 7, height: 7, borderRadius: '50%', flexShrink: 0, display: 'inline-block',
                background: ok ? color : optional ? '#3a3a1a' : '#3a2a2a',
              }} />
              {label}
              {optional && !ok && (
                <span style={{ fontSize: 9, color: '#6a6a3a', marginLeft: 2 }}>(isteğe bağlı)</span>
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, justifyContent: 'center', paddingRight: 18 }}>
          ...
          {algo.tagBits > 0 && (
            <span style={{ fontSize: 11, color: algo.color, fontFamily: 'monospace', fontWeight: 600, textAlign: 'right' }}>tag</span>
          )}
        </div>
      </div>

      {/* Grain-128 AEAD specific: NIST core input preview */}
      {isGrain && nodeData.coreInputPreview && (
        <CoreInputSection cp={nodeData.coreInputPreview} />
      )}

      {/* Grain-128 AEAD specific: LFSR / NFSR register visualization */}
      {isGrain && (
        <>
      ...
      <HexField label={`Auth Tag (${algo.tagBits}-bit)`} value={nodeData.tagOutput} color={algo.color} />
      {isGrain && nodeData.nistCiphertextWithTag && (
